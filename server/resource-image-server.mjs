...
async function readBody(req,limit){const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>limit)throw Error('图片超过 15KB');chunks.push(chunk)}return Buffer.concat(chunks)}
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,'http://127.0.0.1');if(req.method!=='POST'||url.pathname!=='/resource-image')return send(res,404,{error:'not found'});if(!await authenticatedFollower(req))return send(res,403,{error:'没有上传权限'});const key=url.searchParams.get('key');if(!['inventory','logistics'].includes(key))return send(res,400,{error:'资料类型无效'});const type=String(req.headers['content-type']||'');if(!['image/webp','image/jpeg','image/png'].includes(type))return send(res,415,{error:'仅支持 WebP、JPEG、PNG 图片'});const bytes=await readBody(req,15*1024),extension=type==='image/webp'?'webp':type==='image/jpeg'?'jpg':'png',relativePath=`resources/${key}/${randomUUID()}.${extension}`,target=path.join(uploadRoot,...relativePath.split('/'));await mkdir(path.dirname(target),{recursive:true});await writeFile(target,bytes,{flag:'wx'});return send(res,201,{object_path:`tencent:${relativePath}`,url:`/uploads/${relativePath}`})}catch(error){return send(res,/15KB/.test(error.message)?413:500,{error:error.message||'上传失败'})}});
